...
// NewServer returns a new Server object and initializes the listener channels
...
// Run starts listening on the assigned port and bootstraps the message and connection handlers
func (s *Server) Run() {
...
// HandleUserMessages keeps track of joining/leaving clients and broadcasts the new messages to
...
// HandleConnection adds a new client to the list of new joins and connects the input and output
// messages to their respective targets
...
// ProcessOutboundMessages writes the registered messages from the given channel to the client connection
...
// SendToServer registers the given text to be sent to the active clients
func (s *Server) SendToServer(text string) {
	s.chatMessages <- text
}

// SendToClient registers the given text to be sent to the given client channel
